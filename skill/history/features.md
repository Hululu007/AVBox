...
## 轨道与字幕记忆按"身份"重做,五项一起(2026-09-25,用户报"切换音轨后换集能否保留")
...
- **对拍结论(写进活规范 §6.1)**:生产级做法是"清单声明(HLS DEFAULT/AUTOSELECT+LANGUAGE、DASH Role=main/@selectionPriority)→ 用户语言偏好(约束式,跨条生效)→ 按内容记忆的**显式选择**(Emby 4.8/Jellyfin 10.10 按**语言**匹配并回退"同剧最近一集";Plex 只按文件记)→ 会话态(不落库)"四层。本仓库自带参考 `示例文件/TV-fongmi` 已按正确方式实现:Room 表 `(key,type)` 唯一 + 值 = `describeFormat()` 指纹 + 起播 `onTracksChanged` 里重新下发。
...
- **独立审查(用户要求"审查是否有错误遗漏和引入新回归",由子代理只读复审全 diff)**:报出 2 高 4 中 4 低,逐条核实后**全部成立**,已修:
...
- **注释精简(用户要求)**:按项目规范去掉日期、"旧实现…现在…"演进对比、外部产品(如 Plex)出处与章节号,只留"不写会再踩的坑";`TrackMemory` 类注释压到 12 行、方法注释多为单行,纯逻辑类的重载 `field(String)/field(String,int)` 改名 `field/fieldAt` 以免混淆。
...
## 弹幕 API 卡片不再暴露链接(2026-09-25,用户要求)
...
## 修复:「添加 py 源」加载忽长忽慢 —— 预热独占队列 + 单项超时(2026-09-26,用户报"添加py源的加载时间忽长忽慢,这是网络阻塞吗,抓日志看看")
...
- **验证**:`:app:assembleDebug` BUILD SUCCESSFUL;`:app:testDebugUnitTest` **343 用例 / 0 失败**(无新单测:排队/超时是线程行为,现有测试集为纯 JVM、拿不到 `App`)。**真机待走查(用户)**:开机后立刻「添加订阅 / 从本地选择 py」,期望不再有几十秒等待;若仍慢,比对 `echo-warm-spider timeout/drop/stop` 与 `echo-apiurl` 的时间差即可区分"还在排队"与"源自己的网络"。
...
## 修复:竖屏详情页"闪退回首页" —— 空详情不再自动关页 + 退页留痕(2026-09-26,用户报"竖屏影视详情页面可能出现闪退回首页,具体怎么触发的也不清楚,很玄学,抓日志看看")
...
## UI:四个二级页的分组卡片加「卡外左上角小标题」(2026-09-26,用户"给几个二级页面,主题设置,播放设置,偏好设置,预载设置这些里面的分组卡片的左上角加上小标题")

- **需求与两次拍板**:①标题形式由用户在三种里选定 —— **卡片外部左上角小标签**(不是卡内标题行、也不是卡内灰色小字);②主题设置页那两组**已经在卡内首卡左上角**有标题行(「主题颜色」/「液态玻璃效果」+「重置」),用户选定**上移到卡外**,即全站只保留一层标题;③8 个分组文案按提议定稿(见下表)。**设置 tab(§4.3)不在此次范围**:该页的组标题是 2026-09-09 用户明确要求移除的,本次不动。
...
- **审查(同日,SKILL 两轴记账)**:**既有/口味** —— ①`theme_preset_palette` 的值也是「主题颜色」(与 `theme_color` 同文案双 key,check_keys 已报),于是主题设置页组1 会同时出现组标题「主题颜色」与预设色卡那一行的行标题「主题颜色」;这个重复**改动前就存在**(当时是卡内标题行 + 行标题)。**当日已按用户裁决处理**(用户先答"把这个主题颜色这一行卡片删除吧",追问"删哪个"后选定"只删卡内那行标题、保留色卡"):`PresetSeedsRow` 的 `Text(theme_preset_palette)` + 12dp `Spacer` 与外层 `Column` 一并删除,该卡直接显示色卡网格;根因是**简体值填错**(英文 `Preset Palette`、繁体「預設色卡」都对,只有简体写成「主题颜色」),行标题删掉后该 key 无引用,三语声明一并删除(`values` / `values-en` / `values-b+zh+Hant` 各 1 行)。②`toast_permission_required` 声明后无引用,是既有死键(未处理)。**本次引入、记账不改** —— 标题行含 `TextButton` 时整行高约 40dp(纯文字时约 16dp),主题设置页组2 的首卡因此比其它组低约 24dp,属该组有卡外操作按钮的必然结果;预设色卡卡去掉了标题行 ⇒ 该卡高度减少约 28dp。
...
- **第三轮:自查发现并修掉一处"改到了用户没点名的界面"的回归(同日)** —— `SettingsGroup` 是共用组件,**直播设置面板也在用它且传了非空 title**(`ui/activity/LiveScreens.kt` 的 `SettingsGroup(title = group.groupName…)`,最长那条还拼了「长按可删除」提示)。我把标题行从裸 `Text` 改成 `Row` 时给行尾加了 4dp 内边距(为的是让 `TextButton` 与卡片内容 16dp 对齐),等于**把标题可用宽度收窄 4dp** ⇒ 长分组名可能换行位置变化。修法:`end` 改成 `if (action != null) 4.dp else 0.dp` —— 无 action 的纯标题分组回到与改动前逐像素一致(宽度、内边距、行高都不变)。**教训**:共用组件加"为某个调用点服务"的内边距/尺寸时,先列出全部调用点(`grep "SettingsGroup("`)并确认它们不受影响;本轮其余调用点(`BottomSheet` / `ConfigManagePage` / `SearchSettingsSheet` / `HomePage` / `SettingsPage`×3)都是 `title = null`,不受影响。**第三轮验证**:`:app:assembleDebug` BUILD SUCCESSFUL;`:app:testDebugUnitTest` **346 用例 / 0 失败**。
...
- **第四轮(同日,用户发真机截图):液态玻璃那组改回原样 + 两个标签改名** —— 用户看截图后要求:①「液态玻璃」这张卡**改回原来的样子**("android13 上面加上液态玻璃效果"),即卡内首行恢复为「液态玻璃效果 + 重置」、其下是「Android 13 及以上支持液态玻璃」提示,**重置按钮放回卡内**;②卡外那个标签由「液态玻璃效果」改为**「应用效果」**(避免与卡内标题撞文案);③播放设置页标签「播放内核与画面」→**「内核与画面」**。改法:组2 的 `SettingsGroup` 回到 `title = stringResource(R.string.theme_app_effects)`、卡内恢复原来的 `Column { Row { 标题 + TextButton(重置) } + 提示 }`(照抄 HEAD 版原文);`SettingsGroup` 的 `action` 槽**整个撤除**(重置回卡内后无消费者),文件 `git diff` 归零 = **完全回到仓库原状** —— 这同时让第三轮那条"共用组件行尾内边距收窄直播设置面板标题宽度"的隐患彻底消失(不再是"条件式规避",而是组件没被改过)。文案:`theme_app_effects` 新增(应用效果 / App Effects / 應用效果,HK 差异层不需要覆盖),`settings_group_play_picture` 的简体改「内核与画面」、繁体改「核心與畫面」(英文 `Engine & Picture` 本来就不含 Playback,不动)。**验证**:`:app:assembleDebug` BUILD SUCCESSFUL;`:app:testDebugUnitTest` **346 用例 / 0 失败**;`i18n_gate.py` 0/0;`i18n_check_keys.py` 声明 438 / 引用 437(仅既有死键)、`SAME-VALUE-MULTI-KEY(0)`;`i18n_align.py` en / `b+zh+Hant` / `zh-rHK --subset` 全 PASS;`prune_imports.py` 0 未使用 import。**未真机验证**。

## 特性:竖屏视频点全屏直接进竖屏全屏(2026-09-26,用户"能否通过识别视频是竖屏还是横屏的,如果是竖屏的点击全屏控件后直接进入竖屏")
...
- **两个必须同步修的坑(不修则功能不可用)**:
  ...
  2. **rotating 语义**:竖屏→竖屏进全屏**不触发** `onConfigurationChanged`,原式 `full != landNow` 会置 `rotating=true` 且永不复位;此后用户把手机转横屏,`isFullBox()` 的 `!landNow` 翻成 false ⇒ 布局退回预览态但 `fullScreen` 仍 true。改后竖屏视频进全屏 `rotating` 不置位。
...
- **未动**:`LivePlayActivity`(用户指定只做点播详情页);`ComposeVideoController.onRotateClicked`(底栏旋转按钮语义仍合理:竖屏全屏下按它转横屏);UI 未改一行(复用现有 `state.isPortrait` 分支 + 全屏态 `setPreviewMode(false)`);未加任何文案 ⇒ 无多语言改动。
