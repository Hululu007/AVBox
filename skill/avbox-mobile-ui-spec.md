...
- **播放器对话框 M3 化(2026-09-16)**:点播覆盖层的全部 Dialog(内核/倍速/画面尺寸选择、选集、投屏、弹幕设置/搜索、字幕设置/搜索)统一走 M3 语义色与形状 —— 面板 `surfaceContainer` + **18dp 圆角**(2026-09-21 用户定稿:28dp → 16dp → 18dp,含选集侧边面板的起始侧)+ 6dp 阴影;选项 `surfaceBright`,选中 `primaryContainer`/`onPrimaryContainer`,聚焦 `surfaceContainerHighest` + `primary` 2dp 描边;提示文字 `onSurfaceVariant`;输入框 `surfaceContainerHighest` + `outlineVariant`(聚焦 `primary`)描边;选集面板只圆起始侧(贴屏幕上下边)。骨架集中在 `player/ui/PlayerSheets.kt`(全部面板共用),`PlayerSelectDialog` 改为复用同一骨架(不再自带一套配色);res 的旧 `dialog_*` 深色描边调色板随之下线。**字号/尺寸仍走 AutoSize(mm) 档**(`playerDim`/`playerTextSize`):覆盖层按屏宽等比缩放,换 M3 固定 sp 会在电视上明显偏小;交互层(焦点链 + `tvConfirmKey` + 触摸点按 + 默认聚焦项)保持不变。
