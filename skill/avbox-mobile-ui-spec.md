...
- **「接口线路」行(点播仓的第二个入口,2026-09-21 修好可见性)**:位于「默认启动页 / 历史记录上限 / 清除缓存 / DOH」那组之下的**独立单卡**,**仅当当前点播源来自多仓**(`apiLineVisible = HistoryHelper.isApiLineUrl(apiLineUrl)`)时出现,点开是 `AVBoxOptionSheet` 列出仓里的子源。⚠️ 它长期"永远不显示":`SettingsState` 只在 `SettingsViewModel` 构造时读一次 KV,而"当前源来自仓"是**异步**加载后才成立的 ⇒ 切完源退回设置页读到的是"仓地址 + 空线路列表"的旧快照。已补两条幂等刷新:`LifecycleEventEffect(ON_RESUME)` 与 `AppBootstrap.state` 变 `Ready` 时各调一次 `vm.refreshState()`(只重读 KV;缓存大小仍只在 ON_RESUME 走 `refreshCacheSize()`,避免多跑一次缓存目录全量遍历)。仓的**主入口**是配置管理页右上角的「换仓」圆钮(见 §4.7),本行保留作为点播侧的顺手入口。
...
- **换仓入口的可见性必须在页内刷新(2026-09-21 修)**:入口可见性(`canSwitchRepo`)与各卡的「使用中」标记都建立在 `activeUrl`/`liveActiveUrl`/`liveFollow` 上,而它们是 `remember` **只在首次组合读一次**的快照;多仓的解析与地址改写发生在**异步** loadConfig 里(仓地址 → 仓内首条子源,`ApiConfig.switchApiCollectionIfNeeded`),**全程不产生任何 Compose 状态变化** ⇒ 页面不重组,表现为"添加完仓源要退出本页再进来,右上角才出现换仓图标"。补两条幂等刷新途径:①`LifecycleEventEffect(ON_RESUME)` —— 直播仓的改写发生在**直播页**拉取配置时(独立直播源只 `invalidateLiveConfig()`,真正拉取要等进直播页),另覆盖从本地文件选择器返回;②`LaunchedEffect(boot)` 在 `AppBootstrap` 落地 `Ready` 时重读 —— 点播仓的改写就在本页后台完成,已核 `AppBootstrap.startInit` 顺序:**改写先于 Ready**,故能兜住"人一直停在配置管理页"。**只重读这三份"当前态",不重读订阅列表**:订阅列表的增删改都同步写 KV,本地值不会与 KV 分叉,重读反而会与 manageMode 的勾选集错位。⚠️ **不要**把可见条件放宽成 `isApiLineSource(activeUrl)` 来"绕过"刷新 —— 那会让按钮在 Ready 之前就出现,而 sheet 里的仓列表还是空的(白点一次)。同类先例 = 设置 tab「接口线路」行的双通道刷新(§4.3)与 §6.9。
...
- ⚠️ **"Composable 只在首次组合读一次 KV" + "写 KV 发生在异步加载里" = 页面不刷新**。凡是**可见性/选中态/摘要文案**依赖异步改写后的 KV(多仓的仓地址 → 仓内首条子源改写、配置拉取后才成立的标记)的页面,都必须显式补一条刷新通道,否则只能靠"退出重进"(重建 Activity/ComposeView,`remember` 重跑)才正确。**标准配方 = 两条幂等刷新**:①`LifecycleEventEffect(ON_RESUME)`(覆盖"改写发生在别的页面"与"从系统选择器返回");②`LaunchedEffect(AppBootstrap.state)` 在 `Ready` 时重读(覆盖"人就停在当前页、改写由后台完成")—— 前提是**改写先于 `Ready`**(已核 `AppBootstrap.startInit`:`awaitLoadConfig()` → `_state.value = Boot.Ready`)。刷新**只重读"当前态"快照,不要重读用户可编辑的列表** —— 列表的增删改都同步写 KV,重读不会带来新信息,反而会与本地管理态(如 `manageMode` 的勾选集)错位。已落地:设置 tab「接口线路」行(§4.3)、配置管理页「换仓」入口(§4.7)。
