...
- **媒体通知对影视也生效(2026-09-13 用户要求)**:前台服务通知 + MediaSession 原先只对**纯音频**(音乐源)建立,判据写在 `PlayContainer.getAudioOnlyPlayback()`(`有音轨 && 无视频轨`)。现已拆成两个概念,同一个方法不能再混用:
  ...
  - `isAudioOnlyPlayback()` = **三态** `Boolean`(TRUE=纯音频 / FALSE=确定是影视 / **null=取不到轨道信息**):`hostPause()` 用 `!Boolean.TRUE.equals(...)`(只有确定是纯音频才不退后台暂停 —— null 照迁移前一样暂停),封面兜底用 `Boolean.TRUE.equals(...)`(只有确定是纯音频才显示封面)。
  ⚠️ **封面(`artworkView`)与渲染 Surface 同层且盖在其上**(SurfaceFlinger:app 窗口 z > SurfaceView 层):**任何以"有音轨"为条件去 `setArtwork` 的写法都会把视频压成一张海报**(现象:有声音、只有海报,EXO/IJK 一致、与分辨率无关)。因此封面只允许给"纯音频 + 画面未就绪"两个条件同时成立时使用,另有 `MyVideoView.showVideoFrame()` 作为"画面已出即撤封面"的终极互斥兜底;`clearArtwork()` 还必须 `dispose()` 掉 Coil 的在途请求(Coil 的 `onSuccess` 不检查视图可见性)。详见 `history/features.md` 的回归修复条目。
