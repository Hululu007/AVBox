...
- **「接口线路」行(点播仓的第二个入口,2026-09-21 修好可见性)**:位于「默认启动页 / 历史记录上限 / 清除缓存 / DOH」那组之下的**独立单卡**,**仅当当前点播源来自多仓**(`apiLineVisible = HistoryHelper.isApiLineUrl(apiLineUrl)`)时出现,点开是 `AVBoxOptionSheet` 列出仓里的子源。⚠️ 它长期"永远不显示":`SettingsState` 只在 `SettingsViewModel` 构造时读一次 KV,而"当前源来自仓"是**异步**加载后才成立的 ⇒ 切完源退回设置页读到的是"仓地址 + 空线路列表"的旧快照。已补一条幂等刷新:`LifecycleEventEffect(ON_RESUME)` 时调一次 `vm.refreshState()`(只重读 KV;缓存大小也在这一处走 `refreshCacheSize()`,避免多跑一次缓存目录全量遍历)。**这里不订阅仓变更信号**:本页是 `MainScreen` 的 tab,而改写只发生在配置管理页(独立 Activity)或开机阶段,离开本页期间必然已经结束,回本页重读一次就够 —— 同一件事不要有两条刷新路径(同日收窄,见 §4.7 与 §6.9)。⚠️ **不要拿 `AppBootstrap.state` 变 `Ready` 当触发器** —— 它要等 jar 装载也跑完,那时改写早已结束(同日返工,见 §4.7 与 §6.9)。仓的**主入口**是配置管理页右上角的「换仓」圆钮(见 §4.7),本行保留作为点播侧的顺手入口。
