...
- **本地文件选择 = 系统 SAF**(2026-09-11 五轮定稿;**2026-09-16 改"直引原文件优先"**):`ActivityResultContracts.OpenDocument()`(MIME `*/*`)→ Uri 转 `clan://` 接口地址**回填链接输入框**(不直接保存)。打开选择器前先确保存储权限(Android 11+ = 系统「所有文件访问」,跳设置页;**未授权则每次点都跳**;用户拒绝/直接返回 ⇒ Toast「未开启『所有文件访问』,已取消导入(可再点一次重试)」并**取消本次导入**、不再弹选择器 —— 复制分支下同目录引用会静默失效,不替用户做这个决定)。转换规则 `LocalConfigHelper.importLocalConfig`(入口 `handleLocalConfigResult`):**能算出原目录地址就直引,算不出才复制** —— 有权限 + 能解析出真实路径(**不先问 `DocumentsContract.isDocumentUri`** —— 它要经 PackageManager 确认 authority 是文档 provider,在包可见性/部分 ROM 上会对合法文档 Uri 判 false,于是掉进无 DATA 列的 `getDataColumn` ⇒ 能直引的错走复制;改按 authority 逐一解析:externalstorage 的 `primary:`/SD 卡、downloads 的 `raw:`/`msf:`(三段回退:Downloads→Files→`Download/<显示名>`)、media 的 `document:`、**未知 provider 按 docId 前缀兜底**、**第三方文件管理器 FileProvider 的 `…/extfiles/<外置存储相对路径>` 形态**(vivo 文件管理器实测就是这种:无 docId 也无 DATA 列)**取 Uri 里的路径段**)+ 路径存在可读 + 落在 `/sdcard` 下(`RemoteServer` 的 `/file/` 只服务该根);直引才能让配置里 `./x.jar`、`../lib/x.js` 这类**同目录引用**解析到原目录。否则先看**能否算出原目录地址**(能解析出真实路径且落在 `/sdcard` 下):能算出但应用读不到(权限没生效 / ROM 限制)⇒ 走**目录授权直引** —— Toast「本地源要直引原目录,得给一次目录授权:请选择该配置所在的文件夹」→ `OpenDocumentTree`(`ConfigManageActivity.sourceTreeLauncher`/`handleLocalSourceTreeResult`)→ `LocalSourceTree.remember` 持久化授权,**地址仍指向原目录、文件一个都不搬**;本地服务(`RemoteServer` 的 `/file/`)读不到文件时用该授权经 SAF 读出(`LocalSourceTree.open`,靠 `relativeUnder` 把请求路径映射到授权目录),授权列表存 KV `local_source_trees`,重复导入同一源直接复用(`covers`)不再要授权。算不出地址(第三方 provider / SD 卡等非主卷)⇒ 复制到外置私有 `files/config/`(SAF 自带读取授权,**复制不需要存储权限**):有 `./` 引用落 `<md5(uri)>/<原名>` 并把被引用文件按原相对结构搬过来(`./x` 会被重写成"配置所在目录"的 http 前缀,基准正是该子目录;≤32MB/文件、≤64MB 总量,`..` 引用不搬;先 File API `copyRefs`,读不到再用同一份目录授权 `copyRefsFromTree`),无 `./` 引用则沿用 `<md5(uri)>_<原名>`(md5 前缀防同名配置互相覆盖)。⚠️ 判断"应用到底读不读得到原目录"**要看应用内埋点**(`echo-local-src path … src=`):`adb run-as … ls` 会被目录 mode/进程域误导(实测该目录 0770/0660、跑 run-as 报 Permission denied,而应用内 `File.canRead` 为真)。⚠️ 落点是 files **不是 cache**:系统「清除缓存」会把副本删掉,之后只能静默回落到 `filesDir` 旧快照(用户改本地 json 不生效且无提示)。**本地源无权限时拉取失败不再回落旧快照**(`ApiConfig.isLocalSourceUnreadable`,只判 `clan://localhost/`/`file://` 本机形态,`clan://<ip>/` 局域网源不在此列):直接报「本地源文件读不到 / 请开启『所有文件访问』后重试(或重新导入本地源)」—— 避免"改了 json 不生效却毫无提示"。自绘 `LocalFileActivity` 已整页删除。
...
- **轨道选择:同一 track type 只允许一路渲染器持有选择(2026-09-22)**:`ExoPlayer.setTrack` 设目标渲染器前必须清掉**同类其余渲染器**的 `SelectionOverride` —— media3 的 `DefaultTrackSelector.findDefinitionForType` 只取第一个同类 definition、**不会**清其余,于是两路音频渲染器同时 enable,`DefaultMediaClock.onRendererEnabled` 抛 `IllegalStateException("Multiple renderer media clocks enabled.")`,表现为播放中切音轨立刻「视频播放出错」+ 同地址重播,且因切轨前已写音轨记忆 ⇒ 重播后再切必复现。清掉即自动 disable;⚠️ **不要**改用 `setRendererDisabled(其他渲染器, true)` 这类粘性状态(会影响后续换集/换片的自动选轨)。音轨/视轨/内置字幕一律按此办理(本机视频也有两路渲染器:`MediaCodecVideoRenderer` + `ExperimentalFfmpegVideoRenderer`)。
- **音轨记忆必须带渲染器下标(2026-09-22)**:`(组,轨)` 只在所属渲染器内有效 —— `AudioTrackMemory` 的 `_exo_renderer` 键必须与 `_exo_group`/`_exo_track` 一起读写;按"第一个音频渲染器"还原会把记在 ffmpeg 扩展渲染器上的选择套到 MediaCodec 渲染器上(静默选错轨,表现为"选了第二条,换集回来又是第一条")。旧键无渲染器时回落第一个音频渲染器。
