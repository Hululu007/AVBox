...
- **「接口线路」行(点播仓的第二个入口,2026-09-21 修好可见性)**:位于「默认启动页 / 历史记录上限 / 清除缓存 / DOH」那组之下的**独立单卡**,**仅当当前点播源来自多仓**(`apiLineVisible = HistoryHelper.isApiLineUrl(apiLineUrl)`)时出现,点开是 `AVBoxOptionSheet` 列出仓里的子源。⚠️ 它长期"永远不显示":`SettingsState` 只在 `SettingsViewModel` 构造时读一次 KV,而"当前源来自仓"是**异步**加载后才成立的 ⇒ 切完源退回设置页读到的是"仓地址 + 空线路列表"的旧快照。已补两条幂等刷新:`ApiLineSignal` 变更时与 `LifecycleEventEffect(ON_RESUME)` 时各调一次 `vm.refreshState()`(只重读 KV;缓存大小仍只在 ON_RESUME 走 `refreshCacheSize()`,避免多跑一次缓存目录全量遍历)。⚠️ **不要拿 `AppBootstrap.state` 变 `Ready` 当触发器** —— 它要等 jar 装载也跑完,那时改写早已结束(同日返工,见 §4.7 与 §6.9)。仓的**主入口**是配置管理页右上角的「换仓」圆钮(见 §4.7),本行保留作为点播侧的顺手入口。
...
- **换仓入口的可见性必须在页内刷新(2026-09-21 修,同日返工一次)**:入口可见性(`canSwitchRepo`)与各卡的「使用中」标记都建立在 `activeUrl`/`liveActiveUrl`/`liveFollow` 上,而它们是 `remember` **只在首次组合读一次**的快照;多仓的解析与地址改写发生在**异步** loadConfig 里(仓地址 → 仓内首条子源,`ApiConfig.switchApiCollectionIfNeeded`),**全程不产生任何 Compose 状态变化** ⇒ 页面不重组,表现为"添加完仓源要退出本页再进来,右上角才出现换仓图标"。**刷新信号必须由改写点直接发**(`util/ApiLineSignal`,订阅后重读),不能靠反推"加载什么时候完成":`AppBootstrap` 的 `Ready` 要等**配置加载 + jar 装载**两段都跑完,而改写早在第一段就结束了 ⇒ 拿 `Ready` 当触发器会晚几秒到几十秒(真机上表现为"修了等于没修");`Boot.Error` 时更是永远不触发。另配 `LifecycleEventEffect(ON_RESUME)` 兜"改写发生在别的页面"(直播仓要进直播页拉配置时才改写)。**只重读这三份"当前态",不重读订阅列表**:订阅列表的增删改都同步写 KV,本地值不会与 KV 分叉,重读反而会与 manageMode 的勾选集错位。⚠️ **不要**把可见条件放宽成 `isApiLineSource(activeUrl)` 来"绕过"刷新 —— 那会让按钮在仓列表就绪前出现,点开是个空 sheet。设置 tab「接口线路」行(§4.3)是同一处缺陷的另一半,一并换成了信号驱动。
