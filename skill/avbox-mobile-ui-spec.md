...
- **点播 / 直播分段(2026-09-12)**:内容区顶部**常驻**分段行(`CapsuleSegmentedButton` + `style = SegmentStyle.Track`,两段等宽,距屏 16dp、与首卡 12dp),每段两行 = 标题 + `badge` 当前源名(不切分段也能看到两个角色各自的选择);点播段空态 = `LoadStateBox`「暂无订阅」+ **`ic_empty_record` 64dp 空态图标**(2026-09-17 用户要求,与首页引导态/历史/收藏同款);直播段**永不为空**(首项固定为合成的「跟随点播源」卡),但**直播源为空时在跟随卡下方追加 `LoadStateBox`「暂无直播源」+ 同款图标**(`height(220.dp)` 居中,2026-09-17)—— 注意是"追加"不是"替换",否则会丢掉"未配直播源时开启跟随"的能力。
