...
public class Trans {
    ...
    private static class Loader {
        static volatile Trans INSTANCE;
    }

    /** 繁体档位跟随应用语言;KV 未就绪时按"跟随系统"处理,不抛异常 */
    private static boolean wantTraditional() {
        return LanguageManager.INSTANCE.isTraditional();
    }

    /** 按语言签名懒重建:语言变了不能留旧方向的实例(档位就存在实例字段上,避免两处状态不一致) */
    private static Trans get() {
        boolean want = wantTraditional();
        Trans current = Loader.INSTANCE;
        if (current != null && current.trans == want) return current;
        synchronized (Trans.class) {
            current = Loader.INSTANCE;
            if (current == null || current.trans != want) {
                current = new Trans(want);
                Loader.INSTANCE = current;
            }
            return current;
        }
    }

    private Trans(boolean trans) {
        s2t = new HashMap<>();
        t2s = new HashMap<>();
        this.trans = trans;
        if (trans) init();
